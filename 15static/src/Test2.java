...
class BankAccount {
	...
	public void deposit(int amount) {
		...
		System.out.println("[" + accountNumber + "]에 " + amount + "원 입금 완료 (잔액: " + balance + "원)");
	}
	...
	public boolean withdraw(int amount) {
		if(amount > balance) {
			System.out.println("[" + accountNumber + "] 잔액 부족으로 출금 실패");
			return false;
		}else {
			balance -= amount;
			totalAsset -= amount;
			System.out.println("[" + accountNumber + "]에서 " + amount + "원 출금 완료 (잔액: " + balance + "원)");
			return true;
		}
	}
...
public class Test2 {
	public static void main(String[] args) {
		...
		accounts[2] = new BankAccount("박미수", 30000);
		...
		boolean w = accounts[2].withdraw(50000);
		if(w) {
