...
public class Pokemon { //포켓몬 설계도
	...
	public void skill(Pokemon target) {
		if(hp <= 0) {
			System.out.println(name + "은(는) 기절해서 기술을 쓸 수 없습니다!");
			return;
		}
		if(target.hp <= 0) {
		...
		System.out.println(name + "이(가) " + target.name + "을(를) 공격합니다!(공격력: " + power + ")");
	...
	public void status() {
		System.out.println("[포켓몬 상태] " + name + " - 체력: " + hp + ", 공격력: " + power);
	}
