...
class MenuService{
	...
	public List<String> findAll(){
		...
		for(Menu menu : menuList) {
			result.add(menu.name + " " + menu.price + "원");
		}
		return result;
	}
	
	// 매개변수보다 낮은 가격의 메뉴 목록 반환
	public List<String> findUnder(int maxPrice){
		List<String> result = new ArrayList<String>();
		
		result = menuList.stream()
						.filter(m -> m.price <= maxPrice)
						.map(m -> m.name + " " + m.price + "원")
						.collect(Collectors.toList());
		
		return result;
	}
	
	// 가격 오름차순으로 정렬된 메뉴 목록 반환
...
// [4] 주소와 기능을 연결하고, 요청 주소가 들어오면 찾아 실행하는 서비스
class MiniServer {
	Map<String, RequestHandler> mappingTable = new HashMap<String, RequestHandler>();
	...
	public String dispatch(String url, String param) {
		...
		// 주소가 등록되어 있는지 확인
		if(!mappingTable.containsKey(url)) {
			return "404 : " + url + " 은 등록되지 않은 주소입니다.";
		}
...
public class LambdaMvcPreview {
	...
	public static void main(String[] args) {
		...
		server.addMapping("/menu/under", param -> {
			// 입력한 매개변수가 숫자로 이루어진 문자열인지 확인
			if(!param.matches("\\d+")) {
				return "원하시는 가격을 숫자로 입력해주세요.";
			}
			// 매개변수보다 낮은 가격의 메뉴 리스트 반환
			List<String> list = service.findUnder(Integer.parseInt(param));
			String response = param + "원 이하 메뉴 : ";
			for(String str : list) {
				response += str + " / ";
			}
			return response;
		...
		server.addMapping("/menu/sorted", param -> {
			List<String> list = service.findSorted();
			String response = "가격 낮은 순 : ";
			for(String str : list) {
				response += str + " / ";
			}
			return response;
		...
		server.addMapping("/menu/average", param -> "전체 메뉴의 가격 평균은 " + service.findAverage() + "원 입니다.");
		
		System.out.println();
		
		// 등록한 주소 요청
		System.out.println(server.dispatch("/menu", ""));
		System.out.println(server.dispatch("/menu/under", "4500"));
		System.out.println(server.dispatch("/menu/sorted", ""));
		System.out.println(server.dispatch("/menu/average", ""));
		
		// 등록하지 않은 주소 요청
		System.out.println(server.dispatch("event", ""));
	}
