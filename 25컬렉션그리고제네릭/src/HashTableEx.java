...
public class HashTableEx {
	public static void main(String[] args) {
		...
		//1단계 : Hashtable 객체 생성
		// 키, 값이 모두 String 타입의 HashTable 생성
		...
		String capital = table.get("한국");


		//4단계 : null 이 아니면 출력
		if(capital != null) {
			System.out.println("한국 key와 함께 저장된 value: " + capital);
		}


		//5단계 : 저장한 적 없는 key "미국" 으로 꺼내기 시도 후 null 확인
		if(table.get("미국") == null) {
		...
		while(e.hasMoreElements()) {
			// 다음 요소를 현재 keyString 에 저장
			String keyString = e.nextElement();
			System.out.println(keyString + " - " + table.get(keyString));
		}
