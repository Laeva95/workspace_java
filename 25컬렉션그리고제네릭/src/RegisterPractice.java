/*
	...
	■ 문제 0 (서술형) : 아래 두 질문에 주석으로 답하시오.
	   (1) 중복 신청을 걸러내는 용도로 HashSet을 쓰는 이유는?
	       답: HashSet 에는 중복 요소를 저장 할 수 없기 때문에 중복 신청을 걸러낼 수 있음
	   (2) 합격자/대기자 명단으로 ArrayList를 쓰는 이유는?
	       답: ArrayList 는 입력한 순서대로 요소가 저장되기 때문에 합격자 / 대기자 명단에 변화가 있을 경우
	       		해당 내용을 반영하기가 쉬움
...
public class RegisterPractice {
	public static void main(String[] args) {
		...
		for(String str : applyList) {
			if(!checkSet.add(str)) {
				System.out.println("중복 신청 무시 : " + str);
			} else {
		...
		//                주석으로 함께 적으시오!
		// 답: index를 이용해서 요소를 삭제 할 때는 항상 indexOf() 메소드를 이용해서 정확한 index 값을 찾아서 입력해야함
		// 그렇지 않으면 잘못된 요소를 삭제하거나, 배열의 범위를 넘는 index 를 참조해서 에러가 발생 할 수 있음
