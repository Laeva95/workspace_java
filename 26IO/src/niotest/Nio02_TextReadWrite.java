...
public class Nio02_TextReadWrite {
    ...
    public static void main(String[] args) throws IOException {
    	...
    	// Files.writeString(): 전달할 경로의 파일이 없으면 만들고, 이미 있으면 내용을 전부 지우고 새로 덮어씀
    	Files.writeString(path, content, StandardCharsets.UTF_8);
    	
    	System.out.println("member.txt에 기록 완료");
    	...
    	// Files.readString(): 파일의 모든 내용을 읽어서 문자열로 반환
    	String readAll = Files.readString(path, StandardCharsets.UTF_8);
    	
    	System.out.println("읽은 내용 전체 출력");
    	System.out.println(readAll);

    	System.out.println();
    	
    	System.out.println("[3] 줄 단위로 읽기");
    	
    	// Files.readAllLines(): 파일을 줄 단위로 읽어서 리스트로 생성해서 반환
