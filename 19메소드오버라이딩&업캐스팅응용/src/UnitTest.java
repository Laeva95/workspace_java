...
class Unit {
	...
	public void attack(Unit target) {
		...
		if(target instanceof Tank) {
			Tank t = (Tank)target;
			System.out.println("> " + t.name + "는 두꺼운 장갑을 가지고 있다!");
		}
		if(target instanceof Zergling) {
			Zergling z = (Zergling)target;
			System.out.println("> " +  z.name + "은 빠르게 움직인다!");
		}
	}
...
class Marin extends Unit {
	...
	public void stimPack() {
		if(hp <= 10) {
			System.out.println(name + "은(는) 체력이 부족해 스팀팩을 사용할 수 없습니다.");
			return;
		} else {
			super.hp -= 10;
			super.damage += 5;
			System.out.println(name + "이(가) 스팀팩 기능 사용!");
...
class Zergling extends Unit {
	...
	public void rush() {
		super.damage += 3;
		System.out.println(name + "이(가) 돌진한다!");
...
public class UnitTest {
	public static void main(String[] args) {
		...
		if(marin instanceof Marin) {
			Marin m = (Marin)marin;
			m.stimPack();
		}

		//문제 21. zergling 을 Zergling 으로 다운캐스팅해서 rush() 를 호출하시오.
		if(zergling instanceof Zergling) {
			Zergling z = (Zergling)zergling;
			z.rush();
		}
